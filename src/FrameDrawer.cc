...
namespace ORB_SLAM3
{
    ...
    cv::Mat FrameDrawer::DrawFrame(float imageScale)
    {
        ...
        vector<bool> vbVO, vbMap, vbBlur;  // Tracked MapPoints in current frame
        ...
        cv::Scalar odometryColor(0, 255, 0);
        cv::Scalar blurColor(0, 0, 255);
        ...
        else if (state == Tracking::OK) // TRACKING
        {
            ...
            for (int i = 0; i < n; i++)
            {
                if (vbVO[i] || vbMap[i])
                {
                    ...
                    if (vbBlur[i])
                    {
                        cv::rectangle(im, pt1, pt2, blurColor);
                        cv::circle(im, point, 4, blurColor, -1);
                        trackedBlur++;
                    }
                }
            }
            // std::cout << "Drawer Blur Found: " << trackedBlur << std::endl;
        }
    ...
    void FrameDrawer::Update(Tracking *pTracker)
    {
        ...
        // std::cout << "Drawer Total Keypoints: " << N << std::endl;

        mvbVO = vector<bool>(N, false);
        mvbMap = vector<bool>(N, false);

        mvbBlur = vector<bool>(N, false);
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                if (pMP)
                {
                    ...
                    if (!pTracker->mCurrentFrame.mvbOutlier[i])
                    {
                        ...
                        {
                            mvbBlur[i] = true;
                            // std::cout << " Drawer Detected Blur. id: " << pMP->mnId << std::endl;
                        }
